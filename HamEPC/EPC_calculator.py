...
class EPC_calculator(object):
    ...
    def _velocity_force_matrix_G_cal(self, k_grid, q_grid, bands_left_indices, bands_right_indices):
        """
        Args:
            k_grid (list or np.ndarray): (3,)
            q_grid (list or np.ndarray): (3,)
            bands_left_indices (np.ndarray): (list or np.ndarray)
            bands_right_indices (np.ndarray): (list or np.ndarray)
            do_symm (bool): if True, run the EPC_symmetrize
        """
        k_grid, weight = self._get_ir_reciprocal_mesh(k_grid)
        ...
        weight_all = np.split(weight, indices_or_sections=split_sections, axis=0)
        ...
        if self.rank == 0:
            logger = time_logger(self.rank_size, 'velocity_force_matrix_G_cal')
        
        G_tensor_all = []
        for iq, q in enumerate(q_grid):
            temp_G = 0
            for ik, k in enumerate(k_grid_all[self.rank]):
                wave_k = eigen_vecs[ik] # (norbs, norbs)
                eigen_k = eigen[:,ik] # (norbs,)
                
                # calculate the electronic info                    
                kpq = k+q
                eigen_kpq, eigen_vecs_kpq = self._elec_cal(kpq) # (norbs, 1), (1, norbs, norbs)
                wave_kpq = eigen_vecs_kpq[0] # (norbs, norbs)
                eigen_kpq = eigen_kpq[:, 0] # (norbs,)
                
                # cal phase
                phase_k = np.exp(2j*np.pi*np.sum(self.nbr_shift_of_cell_sc*k[None,:], axis=-1)) # shape: (ncells,)
                phase_kpq = np.exp(2j*np.pi*np.sum(self.nbr_shift_of_cell_sc*(k+q)[None,:], axis=-1)) # shape: (ncells,)
                
                # valence bands m
                for ib in bands_left_indices:
                    # conduction bands n
                    for jb in bands_right_indices:
                        
                        wave_coe1 = wave_k[ib] # shape: (norbs,) C^(k)_m
                        wave_coe2 = wave_kpq[jb] # shape: (norbs,) C^(k+q)_n
                        wave_coe3 = wave_k[jb] # shape: (norbs,) C^(k)_n
                        wave_coe4 = wave_kpq[ib] # shape: (norbs,) C^(k+q)_m
                        
                        eigen_diff1 = (eigen_k[ib] - eigen_kpq[jb])**2 # [E^(k)_m - E^(k+q)_n]^2
                        eigen_diff2 = (eigen_kpq[ib] - eigen_k[jb])**2 # [E^(k+q)_m - E^(k)_n]^2
                        
                        tmp1 = np.einsum('a,b -> ab', np.conj(wave_coe1), wave_coe2) # [C^(k)_m]^* * C^(k+q)_n 
                        tmp2 = np.einsum('a,b -> ab', np.conj(wave_coe2), wave_coe1) # [C^(k+q)_n]^* * C^(k)_m
                        tmp3 = np.einsum('a,b -> ab', np.conj(wave_coe4), wave_coe3) # [C^(k+q)_m]^* * C^(k)_n
                        tmp4 = np.einsum('a,b -> ab', np.conj(wave_coe3), wave_coe4) # [C^(k)_n]^* * C^(k+q)_m
                        ...
                        for i_m, m in enumerate(self.cell_cut_list): # ncells
                            for i_n, n in enumerate(self.cell_cut_list): # ncells 
                                M_tensor_1 += np.conj(phase_k[n])*phase_kpq[m]*np.einsum('ab, baij->ij', tmp1, self.grad_mat[i_m,i_n])
                                M_tensor_2 += phase_k[n]*np.conj(phase_kpq[m])*np.einsum('ab, abij->ij', tmp2, self.grad_mat[i_m,i_n])
                                M_tensor_3 += phase_k[n]*np.conj(phase_kpq[m])*np.einsum('ab, abij->ij', tmp3, self.grad_mat[i_m,i_n])
                                M_tensor_4 += np.conj(phase_k[n])*phase_kpq[m]*np.einsum('ab, baij->ij', tmp4, self.grad_mat[i_m,i_n])
                            
                temp_G = temp_G + weight_all[self.rank][ik]*(np.einsum('ij, kl-> ijkl', M_tensor_1, M_tensor_2)/eigen_diff1 - np.einsum('ij, kl-> ijkl', M_tensor_3, M_tensor_4)/eigen_diff2)
                temp_G = np.einsum('i,k,ijkl -> ijkl', np.sqrt(1/atomic_mass), np.sqrt(1/atomic_mass), temp_G)/2.0

            G_tensor_all.append(1.0j*temp_G)
        # shape:(nq, natoms, 3, natoms, 3)
        G_tensor_all = np.array(G_tensor_all).reshape(len(q_grid), self.natoms, 3, self.natoms, 3)
        G_tensor = self.comm.allreduce(G_tensor_all, op=MPI.SUM)
    ...
    def eliashberg_spectrum_cal_for_ME(self, k_grid, q_grid):
        ...
        if q_grid[self.rank].size > 0:
            if self.rank == 0:
                ...
                iq_start = split_sections[self.rank-1]
            # calculate the phonon spectrum in parallel
            q_grid = q_grid[self.rank]
            freq_grid, phon_vecs = self._phonon_cal(q_grid)
            nqs_local = len(q_grid)
        ...
        # k grid is split
        if self.rank == 0:
            print('k grid parallel for eigen solving...')
        split_sections = np.zeros(self.rank_size, dtype=int)
        for i in range(len(k_grid)):
            split_sections[i%self.rank_size] += 1
        split_sections = np.cumsum(split_sections, axis=0)
        ...
        epc_strengths = np.zeros((nqs_local, nmodes))
        if self.rank == 0:
            logger = time_logger(nqs_local, 'eliashberg_spectrum_cal_for_ME')
        # calculate electron-phonon coupling strength
        match_table_save = []
        epc_save = []
        for iq, q in enumerate(q_grid):
                ...
                for ibnd in range(nbands):
                    ...
                    for jbnd in range(nbands):
                        ...
                        for imode in range(nmodes):
                            if freq[imode] < self.phonon_cutoff:
                                continue
                            match_table_save.append([ik, iq, ibnd, jbnd, imode])
                            factor = 1/np.sqrt(2 * self.atomic_mass * abs(freq[imode])) # shape:(natoms,)
                            ...
                            for i_m, m in enumerate(self.cell_cut_list): # ncells
                                for i_n, n in enumerate(self.cell_cut_list): # ncells 
                                    epc += np.conj(phase_kpq[m])*phase_k[n]*np.einsum('mnij,mnij', tmp2, self.grad_mat[i_m,i_n])
                            g2_tmp = np.abs(epc) * np.abs(epc)
                            epc_save.append(g2_tmp)
                            delta_nk = w0gauss((eig_k[ibnd] - self.efermi) * self.inv_smeark) * self.inv_smeark
                            delta_mkq = w0gauss((eig_kpq[jbnd] - self.efermi) * self.inv_smeark) * self.inv_smeark
                            epc_strengths[iq, imode] = epc_strengths[iq, imode] + g2_tmp * delta_nk * delta_mkq * self.weight_k[ik]
            if self.rank==0:
                logger.step(iq+1)

        np.save(os.path.join(self.outdir, f"epc-save-{self.rank}.tmp.npy"), np.array(epc_save))
        # We do not re-split the match tables, since the numbers of match pairs from each core are almost equal.
        if len(match_table_save) > 0:
            match_table_save = np.array(match_table_save)
            match_table_save[:, 1] = match_table_save[:, 1] + iq_start
        else:
            match_table_save = np.empty((0, 5))
        np.save(os.path.join(self.outdir, f"match-table-save-{self.rank}.tmp.npy"), match_table_save)
        # The formula divide by an extra w_qv, but we do not do it here, because it will be eliminated in the a2f calculation.
        epc_strengths = epc_strengths / dos_ef # shape: (nqs[local], nmodes)
        delta_omegas = w0gauss(((freq_grid)[None,:,:] - omegas_list[:,None,None]) * self.inv_smearq) * self.inv_smearq # shape: (nomegas, nqs[local], nmodes)
        a2f = np.einsum('qv, wqv->w', epc_strengths, delta_omegas) / 2.0 * self.weight_q
    ...
    def _aniso_FSR_eliashberg_solve_restart(self, mfreqs:np.ndarray):
        ...
        k_grid, self.weight_k, _, self.full2irr = self._get_ir_reciprocal_mesh(self.k_size, auxiliary_info=True, return_frac=False)
        q_grid = self._get_monkhorst_pack(self.q_size, self.graph_data.latt, return_frac=True)
        ...
        k_grid_all = np.split(k_grid, indices_or_sections=split_sections, axis=0)
        if k_grid_all[self.rank].size > 0:            
            # eigen: (norbs, nks_local) eigen_vecs: (nks_local, norbs, norbs)
            eigen, _ = self._elec_cal(k_grid_all[self.rank])
            eigen = eigen[self.bands_indices,:]
        else:
            eigen = np.empty((nbands, 0))
        # gather eigen & eigen_vecs
        eigen_all = self.comm.allgather(eigen)
        eigen_all = np.concatenate(eigen_all, axis=-1) # (nbands, nks)
        del eigen
        # get the fermi energy and dos of fermi level
        self.efermi = self._get_fermi_level_metal(eigen_all, self.smeark, self.gauss_type, num_electron_include)
        # N(Ef) in lambda formula is the DOS per spin
        ...
        if self.rank == 0:
            print("Fermi energy = {} eV\nDOS of Fermi level (spin factor not included) = {}".format(self.efermi * Hamcts.HARTREEtoEV, dos_ef / Hamcts.HARTREEtoEV))
        include_nk = np.abs(eigen_all.T - self.efermi) < self.e_thr

        match_tables, unique_ik, unique_iq, unique_ibnd, unique_jbnd = self._load_match_table(
            os.path.join(self.outdir, f"match-table-save-{self.rank}.tmp.npy"))

        nmatches = len(match_tables)
        nqs_local = len(unique_iq)
        ...
        if nmatches > 0:
            k_grid = k_grid[unique_ik]
            q_grid = q_grid[unique_iq]
            bands_n_indices = self.bands_indices[unique_ibnd]
            bands_m_indices = self.bands_indices[unique_jbnd]
            freq_grid, _ = self._phonon_cal(q_grid)    
            q_grid = self._frac2car(q_grid)
            epc_save = np.load(os.path.join(self.outdir, f"epc-save-{self.rank}.tmp.npy"))
        else:
            k_grid, q_grid, bands_n_indices, bands_m_indices = None, None, None, None
            freq_grid, eigen_all = None, None

        if self.rank == 0:
            logger = time_logger(nmatches, 'eliashberg_spectrum_cal_for_ME_restart')
        for imatch, each_match in enumerate(match_tables):
            ik, iq, ibnd, jbnd, imode = each_match
            k = k_grid[ik]
            q = q_grid[iq]
            enk = eigen_all[unique_ibnd[ibnd], unique_ik[ik]]
            kpq = k + q
            emkpq, _ = self._elec_cal(np.array([kpq]))
            emkpq = emkpq[bands_m_indices[jbnd], 0]
            g2_tmp = epc_save[imatch]
            delta_nk = w0gauss((enk - self.efermi) * self.inv_smeark) * self.inv_smeark
            delta_mkq = w0gauss((emkpq - self.efermi) * self.inv_smeark) * self.inv_smeark
            epc_strengths[iq, imode] = epc_strengths[iq, imode] + g2_tmp * delta_nk * delta_mkq * self.weight_k[unique_ik[ik]]
            if self.rank == 0:
                logger.step(imatch + 1)

        # The formula divide by an extra w_qv, but we do not do it here, because it will be eliminated in the a2f calculation.
        epc_strengths = epc_strengths / dos_ef # shape: (nqs_local, nmodes)
        delta_omegas = w0gauss(((freq_grid)[None,:,:] - omegas_list[:,None,None]) * self.inv_smearq) * self.inv_smearq # shape: (nomegas, nqs[local], nmodes)
        a2f = np.einsum('qv, wqv->w', epc_strengths, delta_omegas) / 2.0 * self.weight_q
        # The a2f of the whole q is obtained by allreducing the a2f of each process
        if self.comm is not None:
        ...
        if self.rank == 0:
            ...
            fout.close()
        epc_strength = self.epc_strength_cal(a2f, omegas_list)
        logave_freq = self.logave_freq_cal(epc_strength, a2f, omegas_list)
        Tc = self.Allen_Dynes_Tc_cal(epc_strength, self.mius, logave_freq)
        if self.rank == 0:
            print("#"*50+ ' Allen Dynes ' +'#'*50)
        ...
        gap0 = self.BCS_ratio * Tc[0]
        if self.rank == 0:
            print(f"Initial gap from Allen-Dynes Tc is {gap0 * Hamcts.HARTREEtoMEV} meV.")
        
        fgaps = np.zeros((nks, nbands, nmfreqs))
        fgaps[include_nk, :] = gap0
        if self.rank == 0:
            np.save(os.path.join(self.outdir, 'aniso-gap0-function.npy'), fgaps)
        fgaps_iso_old = np.full((nmfreqs), gap0)

        # numbers
        piT = Hamcts.PI * self.temperature
        inv_2piT = 2.0 / self.temperature / Hamcts.PI / self.weight_q
        miu_by_N = self.eff_Coul_pot / dos_ef

        for istep in range(self.eliashberg_iter_max):
            fgaps_sum = np.zeros((nks, nbands, nmfreqs))
            frenorms_sum = np.zeros((nks, nbands, nmfreqs))
            for imatch, each_match in enumerate(match_tables):
                ik, iq, ibnd, jbnd, imode = each_match
                k = k_grid[ik]
                q = q_grid[iq]
                kpq = k + q
                irr_index_kpq = self._get_kpq_irr_index([kpq])[0]
                ekpq, _ = self._elec_cal(kpq)
                ekpq = ekpq[bands_m_indices[jbnd], 0]
                freq = np.abs(freq_grid[iq, imode])
                delta_mkpq = w0gauss((ekpq - self.efermi) * self.inv_smeark) * self.inv_smeark
                g2_tmp = epc_save[imatch]
                tmp1 = np.sqrt(np.power(mfreqs, 2) + np.power(fgaps[irr_index_kpq, unique_jbnd[jbnd], :], 2)) # [nmfreqs]
                # it seems that miu_c is not correct here
                for imfreq in range(nmfreqs):
                    dmfreqs_m = mfreqs[imfreq] - mfreqs
                    dmfreqs_p = mfreqs[imfreq] + mfreqs
                    epc_params_m = g2_tmp * freq * (1.0 / (np.power(dmfreqs_m, 2) + np.power(freq, 2)) - 1.0 / (np.power(dmfreqs_p, 2) + np.power(freq, 2)))
                    epc_params_p = g2_tmp * freq * (1.0 / (np.power(dmfreqs_m, 2) + np.power(freq, 2)) + 1.0 / (np.power(dmfreqs_p, 2) + np.power(freq, 2)))
                    frenorms_sum[unique_ik[ik], unique_ibnd[ibnd], imfreq] += \
                        np.sum(mfreqs / tmp1 * epc_params_m) * delta_mkpq
                    fgaps_sum[unique_ik[ik], unique_ibnd[ibnd], imfreq] += \
                        np.sum(fgaps[irr_index_kpq, unique_jbnd[jbnd], :] / tmp1 * (epc_params_p - miu_by_N)) * delta_mkpq
            if self.comm is not None:
                self.comm.Allreduce(MPI.IN_PLACE, frenorms_sum, op=MPI.SUM)
                self.comm.Barrier()
                self.comm.Allreduce(MPI.IN_PLACE, fgaps_sum, op=MPI.SUM)
                self.comm.Barrier()

            delta_nks = w0gauss((eigen_all - self.efermi) * self.inv_smeark) * self.inv_smeark  # (nbands, nks)
            frenorms_iso = np.einsum('nk, kni, k->i', delta_nks, frenorms_sum, self.weight_k) / 2.0 / dos_ef
            fgaps_iso = np.einsum('nk, kni, k->i', delta_nks, fgaps_sum, self.weight_k) / 2.0 / dos_ef
            frenorms_iso = 1.0 + piT * (frenorms_iso / mfreqs)
            fgaps_iso = piT * fgaps_iso / frenorms_iso
            reldelta = np.sum(np.abs(fgaps_iso - fgaps_iso_old))
            absdelta = np.sum(np.abs(fgaps_iso))
            errdelta = reldelta / absdelta

            fgaps = fgaps_sum / (inv_2piT + frenorms_sum / mfreqs[None,None,:])
            fgaps_iso_old = fgaps_iso
            np.save(os.path.join(self.outdir, 'aniso-gap-function.npy'), fgaps)
            if errdelta < self.eliashberg_iter_thr:
                if self.rank == 0:
                    print("Eliashberg equations reach convergence with {:12.4e}.".format(errdelta))
                break
            else:
                if self.rank == 0:
                    if istep == self.eliashberg_iter_max - 1:
                        print("Warnning: Eliashberg equation solver meet the max step, without convergence.")
                    else:
                        print(f"Eliashberg equation solver: {istep} : {errdelta}")
            if self.comm is not None:
                self.comm.Barrier()
        if self.rank == 0:
            print("################################################## Anisotropic FSR Eliashberg Solver ##################################################")
    ...
    def _generate_k_uniquek_mapping(self, unique_ik):
        for i, each in enumerate(self.full2irr):
            index = np.where(unique_ik==each)[0]
            if len(index) == 0:
                self.full2irr[i] = -1
            else:
                self.full2irr[i] = index

    def _aniso_FSR_eliashberg_solve(self, mfreqs:np.ndarray):
        if self.rank == 0:
            print("################################################## Anisotropic FSR Eliashberg Solver ##################################################")
        k_grid, self.weight_k, _, self.full2irr = self._get_ir_reciprocal_mesh(self.k_size, auxiliary_info=True, return_frac=False)
        q_grid = self._get_monkhorst_pack(self.q_size, self.graph_data.latt, return_frac=True)
        ...
        gap0 = self._initial_gap_function(k_grid, q_grid)
        if self.rank == 0:
            print(f"Initial gap from Allen-Dynes Tc is {gap0 * Hamcts.HARTREEtoMEV} meV.")
        match_tables, unique_ik, unique_iq, unique_ibnd, unique_jbnd = self._load_match_table(
            os.path.join(self.outdir, f"match-table-save-{self.rank}.tmp.npy"))
        ...
        enks, _ = self._elec_cal(k_grid)
        dos_ef = self._get_ef_dos(enks) / 2.0
        include_nk = np.abs(enks[self.bands_indices, :].T - self.efermi) < self.e_thr   # (nks, nbands)
        del enks
        fgaps = np.zeros((nks, nbands, nmfreqs))
        fgaps[include_nk, :] = gap0
        ...
        # numbers
        inv_2piT = 2.0 / self.temperature / Hamcts.PI / self.weight_q
        miu_by_2N = self.eff_Coul_pot / 2.0 / dos_ef
        ...
        for istep in range(self.eliashberg_iter_max):
            ...
            for imatch, each_match in enumerate(match_tables):
                ...
                wave_k = wave_k[0, bands_n_indices[ibnd], :]
                kpq = k + q
                irr_index_kpq = self._get_kpq_irr_index([kpq])[0]
                ekpq, wave_kpq = self._elec_cal(kpq)
                ekpq = ekpq[bands_m_indices[jbnd], 0]
                wave_kpq = wave_kpq[0, bands_m_indices[jbnd]]
                freq = np.abs(freq_grid[iq, imode])
                delta_mkpq = w0gauss((ekpq - self.efermi) * self.inv_smeark) * self.inv_smeark
                g2_tmp = epc_save[imatch]
                tmp1 = np.sqrt(np.power(mfreqs, 2) + np.power(fgaps[irr_index_kpq, unique_jbnd[jbnd], :], 2)) # [nmfreqs]
                for imfreq in range(nmfreqs):
                    dmfreqs = mfreqs - mfreqs[imfreq]
                    epc_params = g2_tmp * freq / (np.power(dmfreqs, 2) + np.power(freq, 2))
                    frenorms_sum[unique_ik[ik], unique_ibnd[ibnd], imfreq] += \
                        np.sum(mfreqs / tmp1 * epc_params) * delta_mkpq
                    fgaps_sum[unique_ik[ik], unique_ibnd[ibnd], imfreq] += \
                        np.sum(fgaps[irr_index_kpq, unique_jbnd[jbnd], :] / tmp1 * (epc_params - miu_by_2N)) * delta_mkpq
            if self.comm is not None:
                self.comm.Allreduce(MPI.IN_PLACE, frenorms_sum, op=MPI.SUM)
                self.comm.Barrier()
                self.comm.Allreduce(MPI.IN_PLACE, fgaps_sum, op=MPI.SUM)
            ...
            fgaps_new = fgaps_sum / (inv_2piT + frenorms_sum / mfreqs[None,None,:])
            dfgap_max = np.max(np.abs(fgaps_new - fgaps).flatten())
            if dfgap_max < self.eliashberg_iter_thr:
                if self.rank == 0:
                    print("Eliashberg equations reach convergence with {:12.4e}.".format(dfgap_max))
                    np.save(os.path.join(self.outdir, 'aniso-gap-function.npy'), fgaps_new)
                break
            else:
                fgaps = fgaps_new
                if self.rank == 0:
                    if istep == self.eliashberg_iter_max - 1:
                        ...
                        print(f"Eliashberg equation solver: {istep} : {dfgap_max}")
            if self.comm is not None:
                self.comm.Barrier()
        if self.rank == 0:
            print("################################################## Anisotropic FSR Eliashberg Solver ##################################################")

    def _iso_FST_eliashberg_solve(self, mfreqs:np.ndarray):
        pass
    ...
    def superconductivity_cal(self):
        ...
        """
        if self.eliashberg:
            self.matsubara_cutoff = self.matsubara_cutoff * self._get_ph_cutoff_freq()
            mfreqs = self._generate_matsubara_frequency(fermion=True)
            if self.anisotropy:
                if self.ME_solver_restart:
                    self._aniso_FSR_eliashberg_solve_restart(mfreqs)
                else:
                    self._aniso_FSR_eliashberg_solve(mfreqs)
            else:
                pass
        else:
            omegas, a2f = self.eliashberg_spectrum_cal()
            if self.rank == 0:
                fout = open(os.path.join(self.outdir, "a2f.dat"), 'w')
    ...
    def _generate_matsubara_frequency(self, fermion:bool=True):
        if fermion:
            # ωj = (2j + 1)πT
            index_max = int((self.matsubara_cutoff / (Hamcts.PI * self.temperature) - 1) / 2)
            mfreqs = np.arange(index_max + 1)
            mfreqs = (mfreqs * 2 + 1) * Hamcts.PI * self.temperature
        else:
            # ωj = 2jπT
            index_max = int((self.matsubara_cutoff / (Hamcts.PI * self.temperature)) / 2)
            mfreqs = np.arange(index_max) + 1
            mfreqs = mfreqs * 2 * Hamcts.PI * self.temperature
        if self.rank == 0:
            print(f"Using {index_max+1} Matsubara frequencies...")
        return mfreqs

    def _get_ph_cutoff_freq(self):
        # phonon cutoff frequency is the largest frequency in Gamma point.
        if self.natoms == 1:
            freqs, _ = self._phonon_cal(np.array([0.5, 0.5, 0.5]))
        else:
            freqs, _ = self._phonon_cal(np.array([0.0, 0.0, 0.0]))
        return np.max(freqs[0])

    def _aniso_epc_param_cal(self, k, wave_nk, q_grid, freq_grid, phon_vecs, delta_mfreqs:np.ndarray, istep:int):
        """
        Args:
            k_grid: (3, )
            q_grid: (nqs_local, 3)
            delta_mfreqs: (nmfreqs, )
            wave_nk: (norbs, )
        """
        # \frac{1}{\hbar}\sum_{\nu} {|g_{mn\nu}(\mathbf{k},\mathbf{q})|}^{2}
        #   \times\frac{2\omega_{\mathbf{q}\nu}}{{(\omega_{j}-\omega_{j_{'}})}^{2}+\omega^{2}_{\mathbf{q}\nu}}  

        nmodes = 3 * self.natoms
        ndmfreqs = len(delta_mfreqs)
        nbands = len(self.bands_indices)
        nqs_local = len(q_grid)

        phase_k = np.exp(Hamcts.JTWOPI*np.sum(self.nbr_shift_of_cell_sc*k[None,:], axis=-1)) # shape: (ncells,)
        epc_params = np.zeros((nqs_local, nbands, ndmfreqs))
        ...
        for iq, q in enumerate(q_grid[self.rank]):
            kpq = k+q
            eig_kpq, eigen_vecs_kpq = self._elec_cal(kpq) # (norbs, 1), (1, norbs, norbs)
            eig_kpq = eig_kpq[self.bands_indices, 0] # (nbands, )
            freq = freq_grid[iq]
            eigen_vec_phon = phon_vecs[iq]

            wave_kpq = eigen_vecs_kpq[0, self.bands_indices] # (nbands, norbs)
            emkq_match_table = np.abs(eig_kpq - self.efermi) < self.e_thr
                ...
                for imode in range(nmodes):
                    ...
                    for i_m, m in enumerate(self.cell_cut_list): # ncells
                        for i_n, n in enumerate(self.cell_cut_list): # ncells 
                            epc += np.conj(phase_kpq[m])*phase_k[n]*np.einsum('mnij,mnij', tmp2, self.grad_mat[i_m,i_n])
                    g2_tmp = np.abs(epc) * np.abs(epc)
                    for idmfreq in range(ndmfreqs):
                        epc_params[iq, jbnd, idmfreq] += g2_tmp * freq[imode] / (np.power(delta_mfreqs[idmfreq], 2) + np.power(freq[imode], 2))
        epc_params *= (2.0 * self.weight_q)
        return epc_params

    def _initial_gap_function(self, k_grid, q_grid):
        # To solve ME, we need to save the epc while AD Tc step.
        # We will split q grid to each core and save their own match table and also flatted epc matrix.
        # We do not need to balance the number of matches for each core, since they will be almost equal.
        omegas, a2f = self.eliashberg_spectrum_cal_for_ME(k_grid, q_grid)
        if self.rank == 0:
            fout = open(os.path.join(self.outdir, "a2f.dat"), 'w')
            for iomega, omega in enumerate(omegas):
                fout.write(f"{str(round(omega * Hamcts.HARTREEtoMEV, 10))}    {str(round(a2f[iomega], 10))}\n")
        ...
        if self.rank == 0:
            ...
            print(miu_Tc_str)
            print("#"*50+ ' Allen Dynes ' +'#'*50)
        return self.BCS_ratio * Tc[0]

    def _get_kpq_irr_index(self, kpqs_cart):
        # k (3, )
        # q_grid (nqs_local, 3)
        index_irr_kpqs = []
        nk1mul2 = self.k_size[0] * self.k_size[1]
        # Add 10^(-10) to safely tranform float to int

        kpqs:np.ndarray = self._car2frac(kpqs_cart)
        kpqs[kpqs < -Hamcts.TENPM10] = kpqs[kpqs < -Hamcts.TENPM10] + 1.0
        kpqs:np.ndarray = np.round(kpqs * np.array(self.k_size)[None,:]) + Hamcts.TENPM10
        kpqs = kpqs.astype(int)
        for i in range(3):
            index_2BZ = kpqs[:, i] >= self.k_size[i]
            kpqs[index_2BZ, i] = kpqs[index_2BZ, i] - self.k_size[i]
        index_irr_kpqs = [self.full2irr[kpq[0]+kpq[1]*self.k_size[0]+kpq[2]*nk1mul2]  for kpq in kpqs]
        # if index_irr_kpqs == -1:
        #     raise RuntimeWarning("The k and q grid is not match with each other.", '7032')
        return index_irr_kpqs
